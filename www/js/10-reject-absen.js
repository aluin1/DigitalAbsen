...
///load 
myApp.onPageInit("10-reject-absen", function(page){
...
var strUsername = localStorage.getItem("LogonUsername"); 
 ...
 $$('.btn-action-approve').on('click', function () {
		...
		var strAction = $$(".txtnoteR").val().replace(/(?:\r\n|\r|\n)/g, '<br>'); 
	...
	if (strAction == ''){
			 myApp.modal({
		 title:  '<div style="margin-bottom:5px"><i style="font-size:40px;color:#047edf" class="mdi mdi-information-outline"></i> </div><p style="color:#047edf;font-size:15px; margin-bottom: 0px;   ">Catatan reject harus diisi</p> ',
		 buttons: [
		    {
 ...
 myApp.hidePreloader();	
    ...
    type: "POST",
    data:' {"username" : "'+strUsername+'"  ,  "id" : "'+strId+'",  "action" : "R",  "noteapp" : "'+strAction+'"}' ,
