...
myApp.onPageInit("7-absen-luar-wr-kantornon", function(page){
    ...
    success: function (data) {
		...
		var dtwr=data.data;
		  for (i = 0; i < dtwr.length; i++) {
			  ...
			  //var dataWorkingSave='<tr><td>'+dtwr[i].WR_Kegiatan+'</td><td>'+dtwr[i].WR_Hasil+'</td><td>'+dtwr[i].WR_Status+'  <span style="padding-left: 50px;"><i style="color: #047edf;font-size: 20px;" class="btn-delete mdi mdi-comment-remove-outline"  "></i></span></td></tr>';	
...
var strUsername = localStorage.getItem("LogonUsername"); 
	 ...
	 var valradios = page.query.valradios;  
	 var strCondition = page.query.strCondition; 
	 var strnoted = page.query.strnoted;  
...
if(valradios=='Other'){
  ...
  myApp.showPreloader("<div style='margin-bottom:-15px'><img src='icon/ic_launcher.png' width='50'></div><p style='color: #047edf; font-size: 12px;  margin: 30px 0 0 ; '>Loading Aplikasi</p>");
    setTimeout(function () { 
		 ...
		 $.ajax({
     url: ""+useWeb+"/Attendance/outofficenon_attend",
    type: "POST",
    data:' {"username" : "'+strUsername+'"  ,  "latitude" : "'+lat+'",  "longitude" : "'+lang+'",  "condition" : "'+strCondition+'", "note":"'+strnoted+'",  "photo":"'+strfoto+'"}' ,
    contentType: "application/json",
	headers: {
    "Authorization": "" + strToken
  },
    success: function (data) { 
     ...
     // url: ""+useWeb+"/Attendance/workingreport_attend",
    // type: "POST",
    // data:' {"username" : "'+strUsername+'"  ,  "wr_keg" : "'+strKeg+'",  "wr_hasil" : "'+strHasil+'",  "wr_status" : "'+valradios3+'"}' ,
    // contentType: "application/json",
	// headers: {
    // "Authorization": "" + strToken
  // },
    ...
    setTimeout(function () { 
     ...
     url: ""+useWeb+"/Attendance/outofficenon_attend",
    type: "POST",
    data:' {"username" : "'+strUsername+'"  ,  "latitude" : "'+lat+'",  "longitude" : "'+lang+'",  "condition" : "'+strCondition+'", "note":"'+valradios+'",  "photo":"'+strfoto+'"}' ,
    contentType: "application/json",
	headers: {
    "Authorization": "" + strToken
  },
